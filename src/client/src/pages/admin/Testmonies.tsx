...
import type { TestomonialList, Testomonial,TestomonialFormData } from "@/types/types";
import { createTestomonial,fetcthTestomonials,removeTestomonialById } from "@/apis/testomonials";
...
export const AdminTestimonials: React.FC = () => {
  ...
  const {mutate:newTestom} = useMutation({
      mutationFn:createTestomonial,
      onSuccess:(data)=>{
        console.log(`Created New Testomonial`);
        queryClient.invalidateQueries({queryKey:["testomonials"]});
        reset();
        console.log(data);
  ...
  const { register, handleSubmit,formState:{errors}, reset, setValue } = useForm<Testomonial>();
        ...
        <>
          ...
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[60] p-4 backdrop-blur-sm">
            <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-xl overflow-hidden border border-gray-100 dark:border-slate-800">
              ...
              <form className="p-6 space-y-4" onSubmit={handleSubmit(onSubmitTestom)}>
                ...
                <div className="grid grid-col-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                   ...
                   <input type="number" min={1} max={5} className="w-full px-4 py-2 bg-gray-50 dark:bg-slate-800 border-gray-200 dark:border-slate-700 rounded-lg outline-none focus:ring-2 focus:ring-[#FF9933] dark:text-white text-sm" 
                   {...register("rating")}
                   />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <textarea
                    {...register("testimonial_text", { required: "Testomony Text is Required"})}
                    rows={4}
                  ...
                  ></textarea>
                   {errors.testimonial_text && <p className="text-sm text-red-500 mt-1">{errors.testimonial_text.message}</p>}
                </div>
